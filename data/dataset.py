...
class DatasetManager:
    def __init__(self, data_list=None):
        self.data_list = data_list or relbench_data_list
    ...
    def download_all(self):
        ...
        for dataname in self.data_list:
            self.download_dataset(dataname)

    def download_dataset(self, database_name):
        """
        Download the specified dataset
        """
        try:
            print(f"Downloading dataset: {database_name}")
            dataset = get_dataset(name=database_name, download=True)
            print(f"{database_name} downloaded.")
            return dataset
        except Exception as e:
            print(f"Failed to download {database_name}: {e}")

    def download_task(self, database_name, task_name):
        """
        Download the specified task
        """
        try:
            print(f"Downloading task: {task_name} from {database_name}")
            task = get_task(database_name, task_name, download=True)
            print(f"Task {task_name} downloaded.")
            return task
        except Exception as e:
            print(f"Failed to download task {task_name}: {e}")

    
    def get_task_description(self, database_name, task_name):
        """
...
if __name__ == "__main__":
    Data = DatasetManager()
    Data.get_task_description('rel-f1', 'driver-top3')
